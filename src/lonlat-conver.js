...
const gcj02ToWgs84 = function (lon, lat) {
  ...
  return { lon: lngtitude, lat: latitude };
};
...
const tubaToWgs84 = function (lon, lat) {
  ...
  return { lon: x2 / 100000.0, lat: y2 / 100000.0 };
};
...
const wgs84ToTuba = function (lng, lat) {
  ...
  return { lon: _X / 100000.0, lat: _Y / 100000.0 };
};
...
const tubaToBaidu = function (lng, lat) {
  // 1. 图吧 转 GPS（WGS-84）
  const wgs84 = tubaToWgs84(parseFloat(lng), parseFloat(lat));
